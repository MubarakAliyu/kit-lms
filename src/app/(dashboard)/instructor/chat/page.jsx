...
export default function InstructorChatPage() {
  ...
  useEffect(() => {
    getConversations(INSTRUCTOR_ID)
      .then((list) => {
  ...
  return (
    <div className="flex h-[calc(100vh-9rem)] overflow-hidden rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)]">
      ...
      >
        {active ? (
          <ChatThread conversation={active} onBack={() => setActiveId(null)} />
        ) : (
          <EmptyChatState />
        )}
      </section>
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 p-10 text-center">
      ...
      <p className="text-sm text-[var(--text-secondary)]">
        Pick a student conversation from the list.
      </p>
    </div>
  );
}
...
function MessageBubble({ message }) {
  const sent = message.sender_id === INSTRUCTOR_ID;
  return (
    <motion.li
