...
import { updateAssignment } from "@/_lib/api/instructor";
import RichLessonEditor from "@/_components/editor/RichLessonEditor";

const SUBMISSION_TYPES = [
  { key: "text", label: "Written Answer", description: "Type a response", icon: Pencil, disabled: false },
  { key: "link", label: "Link Submission", description: "URL to work", icon: LinkIcon, disabled: false },
  { key: "file", label: "File Upload", description: "Coming soon", icon: Upload, disabled: true },
];
...
// Convert an ISO-ish deadline ("2025-05-10" or "2025-05-10T12:00:00Z") into the
// YYYY-MM-DDTHH:mm local-time string that <input type="datetime-local"> needs.
...
function defaultsFromAssignment(a) {
  return {
    title: a?.title ?? "",
    instructions: a?.instructions ?? "",
    submission_types: a?.submission_types?.length
      ? a.submission_types
      : a?.submission_type
      ? [a.submission_type]
      : ["text"],
    ...
    grade_type: a?.grade_type ?? "letter",
    allow_late: !!a?.allow_late,
    late_penalty: a?.late_penalty ?? 0,
  };
}
...
export default function EditAssignmentModal({
  ...
  useEffect(() => {
    if (!isOpen) return;
    reset(defaultsFromAssignment(assignment));
  ...
  }, [isOpen, assignment, onClose, reset]);
            ...
            >
              <div className="flex flex-1 flex-col gap-6 overflow-y-auto px-5 py-5">
                <Section title="Basic Info">
                  <Field id="edit-ass-title" label="Assignment Title" error={errors.title?.message}>
                    <input
                      id="edit-ass-title"
                      type="text"
                      {...register("title")}
                ...
                <Section title="Grading">
                  <Field id="edit-ass-grade-type" label="Grade Type">
                  ...
                  <Field id="edit-ass-late" label="Allow Late Submissions">
                    <Controller
                      name="allow_late"
                      control={control}
                      render={({ field }) => (
              ...
              <footer className="flex items-center justify-between gap-2 border-t border-[var(--border-color)] px-5 py-3">
                <span className="text-[11px] text-[var(--text-muted)] font-mono-ui">
                  {assignment.course_title ? `Course: ${assignment.course_title}` : ""}
                </span>
                <motion.button
                  type="submit"
                  whileTap={{ scale: 0.97 }}
